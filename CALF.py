...
files_spectra = [f for f in all_files if f[-14:-4] == '_continuum' and f[-4::]=='.txt']

#files_spectra = [files_spectra[8]]
	...
	for strength in logN:
		...
		for loop in range(int(number_loops_QSO)):
			for i in range(len(files_spectra)):
				...
				max_noise_index = np.max(np.where(flux/err > 3)) 
				redshift = np.random.uniform((1+z_QSO)*1026./1216. - 1 , np.max((wave[max_noise_index]/CIV_1-1, z_QSO)),1)
				amplitude = logN_to_amp(strength,b,(1+redshift)*CIV_1)
				# Create modified flux
				mod_flux = add_CIV(wave,flux,redshift,amplitude,b/RESVEL)
